...
void loop()
{
  cycletime = millis(); // dynamic variable that is going to be called multiple times, so I call it here once, might never even be remotely needed

  if (digitalRead(rotButton) == LOW and (cycletime - last_rotButton) > debounce_time)
  { // mess of code that is responsible for setting sealevel pressure
    if (currentscreen == sealevelpressurechangescreen and select_sealevel == 0)
    { // if currently on changing screen and in screen changing mode
      select_sealevel = 1;
      encoder->setPosition(sealevelpressure);
    }
  ...
  if (currentscreen == sealevelpressurechangescreen and select_sealevel == 1)
  { // if it is being changed, just dump it to the variable
  ...
  { // change screens once per cycle even if scroll is faster
  ...
  { // sets allow sleep to false if it already isn't and id current screen isn't the sleep screen
    gotosleepat = 0;
  }
  if (currentscreen == -1)
  { // in case of sleep screen
    if (gotosleepat == 0)
    {
      gotosleepat = cycletime + time_until_sleep; // calculate time when to go to sleep
    }
    if (gotosleepat != 0 and cycletime > gotosleepat)
    { // send to sleep if sleep is allowed and the time is up
      sleep_enable();
      detachInterrupt(PIN_IN1);
  ...
  case sealevelpressurechangescreen:
  ...
  case 4:
    currentalt = bmp.readAltitude(sealevelpressure);
    if (currentalt > maxheight)
      maxheight = currentalt;
    if (currentalt < minheight)
      minheight = currentalt;
    if (digitalRead(rotButton) == LOW)
      beginheight = minheight = maxheight = currentalt;
    default_screensetup(2);
    display.setCursor(12, 2);
    display.print(abs(currentalt - beginheight));
    if ((currentalt - beginheight) < 0)
      display.drawTriangle(0, 2, 5, 15, 10, 2, SSD1306_WHITE);
    else if ((currentalt - beginheight) > 0)
      display.drawTriangle(0, 15, 5, 2, 10, 15, SSD1306_WHITE);
    display.setCursor(2, 22);
    display.setTextSize(1);
    display.print(maxheight - minheight);
    break;

  case 5: // watchdog to manually reset arduino for example when a sensor on the I2C bus has failed due to bugs etc.
    display.drawBitmap(0, 0, bitmap, imageWidth, imageHeight, WHITE);
    if (digitalRead(rotButton))
      watchdog_begintime = cycletime;
    if ((cycletime - watchdog_begintime) > watchdogtime)
      resetFunc();
    break;
  default:
    break;
